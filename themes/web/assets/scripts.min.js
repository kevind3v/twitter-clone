...
$('html').attr('data-theme','light')}});$(function(){$(document).on('submit',"form:not('.ajax_off')",function(e){e.preventDefault();var form=$(this);var load=$('#loading');var flashClass='ajax_response';var flash=$('#'+form[0].getAttribute('id')+' .'+flashClass);form.ajaxSubmit({url:form.attr('action'),type:'POST',dataType:'json',beforeSend:function(){load.show()},success:function(response){if(response.redirect){window.location.href=response.redirect}else{if(response.message){flash.html(response.message).fadeIn(100)}else{flash.fadeOut(100);window.location.reload()}}},complete:function(){load.hide();if(form.data('reset')===!0){form.trigger('reset')}},})})});;;